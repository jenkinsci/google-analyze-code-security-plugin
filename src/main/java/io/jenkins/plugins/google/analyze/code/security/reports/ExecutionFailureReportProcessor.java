...
public class ExecutionFailureReportProcessor extends ReportProcessor<ErrorReportRequest> {
    ...
    public String generateReport(final ErrorReportRequest errorReportRequest) {
        ...
        content.add(ReportConstants.REPORT_OPEN_HTML.replace(
                /*target=*/ "$REPORT_TITLE$", Config.PLUGIN_ERROR_REPORT_NAME));

        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.ZERO, /*key=*/ "File Scanned", errorReportRequest.getValidationFilePath()));
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.ZERO, /*key=*/ "Start Time(UTC):", errorReportRequest.getScanStartTime()));
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.ZERO, /*key=*/ "End Time(UTC):", errorReportRequest.getScanEndTime()));
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.ZERO, /*key=*/ "Plugin Config:", StringUtils.EMPTY));
        addConfigParams(errorReportRequest.getPluginConfig(), content);
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.ZERO, /*key=*/ "Error Code:", errorReportRequest.getErrorCode()));
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.ZERO, /*key=*/ "Error Message:", errorReportRequest.getError()));
    ...
    private void addConfigParams(final PluginConfig pluginConfig, final List<String> content) {
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.SINGLE, /*key=*/ "Organization ID:", pluginConfig.getOrgID()));
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.SINGLE, /*key=*/ "Scan File Name:", pluginConfig.getScanFileName()));
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.SINGLE, /*key=*/ "Scan File Path:", pluginConfig.getFilePath()));
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.SINGLE, /*key=*/ "Scan Time Out:", String.valueOf(pluginConfig.getScanTimeOut())));
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.SINGLE,
                /*key=*/ "Fail Silently " + "Configuration:",
                String.valueOf(pluginConfig.getFailSilentlyOnPluginFailure())));
    ...
    private void processAssetViolationConfig(final PluginConfig pluginConfig, final List<String> content) {
        if (pluginConfig.getIgnoreAssetViolation().equals(true)) {
            content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                    HTMLIndent.SINGLE, /*key=*/ "Ignore Asset " + "Violation:", String.valueOf(true)));
            return;
        }
        final List<String> violationConfigs = new ArrayList<>();
        violationConfigs.add("Fail Build On Asset Violation : ");
        violationConfigs.add(String.format(
                "{ConfigAggregator : %s}", pluginConfig.getConfigAggregator().name()));
        violationConfigs.add("SeverityConfigs: [");
        if (pluginConfig.getAssetViolationConfigs() != null) {
            pluginConfig
                    .getAssetViolationConfigs()
                    .forEach((config -> violationConfigs.add(
                            String.format("{%s : %s}", config.getSeverity().name(), config.getCount()))));
        }
        violationConfigs.add("]");
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.SINGLE,
                "Asset Violation " + "Configuration:",
                StringUtils.join(violationConfigs, /*separator=*/ " ")));
        content.add(ReportUtils.buildHTMLDivWithKeyAndOptionalValueEntry(
                HTMLIndent.SINGLE, /*key=*/ "Ignore Asset " + "Violation:", String.valueOf(false)));
    }
}
